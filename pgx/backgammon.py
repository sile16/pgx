...
"""
Backgammon environment with optimized implementation.
...
class Backgammon(core.StochasticEnv):
    ...
    def stochastic_action_probs(self, state: State) -> Array:
        ...
        For backgammon, dice rolls are state-independent (uniform distribution).
        """
    ...
    def num_stochastic_actions(self) -> int:
        """Number of possible stochastic outcomes (dice roll combinations)."""
    ...
    def stochastic_step(self, state: State, action: Array) -> State:
        """Backward compatibility alias for step_stochastic."""
    ...
    def _set_dice(self, state: State, dice: Array) -> State:
        ...
        legal_action_mask = _legal_action_mask(state._board, playable_dice, dice, played_dice_num)
        return state.replace(
            _dice=dice,
            _playable_dice=playable_dice,
            _played_dice_num=played_dice_num,
...
def _is_open(board: Array, point: int) -> bool:
    """Check if a point is open (available for a checker to land on)."""
    return board[point] >= -1


def _rear_distance(board: Array) -> int:
    """Calculate the distance of the rearmost checker from the goal."""
    has_checker = board[:24] > 0
    rear_pos = jnp.where(has_checker, jnp.arange(24), 100)
    rear_idx = jnp.min(rear_pos)
    return 24 - rear_idx
...
def _distance_to_goal(src: int) -> int:
    """Calculate distance from a point to bearing off."""
    return 24 - src


def _exists(board: Array, point: int) -> bool:
    """Check if a checker exists at the given point."""
    return board[point] >= 1
...
def action_to_str(action) -> str:
    ...
    src, die, tgt = _decompose_action(jnp.int32(action_int))
    src_int = int(src)
    die_int = int(die)
    tgt_int = int(tgt)

    if src_int == -2:
        return f"No-op (die: {die_int})"
    elif src_int >= 24:
        return f"Bar/{tgt_int + 1} (die: {die_int})"
    elif tgt_int == 26:
        return f"{src_int + 1}/Off (die: {die_int})"
    else:
        return f"{src_int + 1}/{tgt_int + 1} (die: {die_int})"


def stochastic_action_to_str(action) -> str:
    """Convert a stochastic action (dice roll) to a human-readable string."""
    action_int = int(action)
    dice = _STOCHASTIC_DICE_MAPPING[action_int]
    return f"Rolled: {int(dice[0]) + 1}-{int(dice[1]) + 1}"


def turn_to_str(turn: int) -> str:
    """Convert a turn number to a player string."""
    return "Black" if turn == 0 else "White"


# Heuristic observation functions (simplified stubs for backward compatibility)
def _is_race(board: Array) -> bool:
    """Check if the game is in racing phase (no contact between pieces)."""
    # Simplified: check if all black pieces are in home board
    black_home = jnp.sum(board[18:24] > 0) + board[26]
    return black_home == 15


def _can_bear_off_current(board: Array) -> bool:
    """Check if current player can bear off."""
    return _is_all_on_home_board(board)


def _can_bear_off_opponent(board: Array) -> bool:
    """Check if opponent can bear off (from their perspective)."""
    flipped = _flip_board(board)
    return _is_all_on_home_board(flipped)


def _pip_count_current(board: Array) -> int:
    """Calculate pip count for current player."""
    positions = jnp.arange(24)
    pips_per_point = jnp.maximum(board[:24], 0) * (24 - positions)
    return jnp.sum(pips_per_point)


def _pip_count_opponent(board: Array) -> int:
    """Calculate pip count for opponent."""
    positions = jnp.arange(24)
    pips_per_point = jnp.maximum(-board[:24], 0) * (positions + 1)
    return jnp.sum(pips_per_point)


def _pip_count_differential_scaled(board: Array) -> float:
    """Calculate scaled pip count differential."""
    current_pips = _pip_count_current(board)
    opponent_pips = _pip_count_opponent(board)
    return (current_pips - opponent_pips) / 167.0  # Max pip count


def _observe_with_heuristics(state: State) -> Array:
    """Observation with heuristic features (stub - returns fast observation)."""
    return _observe_fast(state)


def _blot_board(board: Array) -> Array:
    """Get mask of blots (single checkers) for current player."""
    return (board[:24] == 1).astype(jnp.float32)


def _blocker_board(board: Array) -> Array:
    """Get mask of blockers (2+ checkers) for current player."""
    return (board[:24] >= 2).astype(jnp.float32)


def _observe_full(state: State) -> Array:
    """Full observation (stub - returns fast observation)."""
    return _observe_fast(state)


def _get_abs_board(board: Array) -> Array:
    """Get absolute board values for visualization."""
    return jnp.abs(board)
