...
"""
Backgammon variant with fast/minimal observation.

Optimization: Skip expensive heuristic computations in observation:
- Skip race detection
- Skip pip count computation
- Skip bear-off checks
- Use vectorized blot/blocker computation

Observation is reduced from 86 to 34 elements (board + dice only).
"""
...
class State(core.State):
    ...
    # Reduced observation size: 34 elements (board + dice only)
...
class BackgammonV2FastObs(core.StochasticEnv):
...
# =============================================================================
# FAST OBSERVATION (skip heuristics)
# =============================================================================
...
def _observe_fast(state: State) -> Array:
    ...
    Observation structure (34 elements):
    - [0:28]  Scaled board (raw / 15)
    - [28:34] Scaled dice counts (raw / 4)

    Skips: race flag, bear-off checks, pip counts, blots, blockers
    """
    board = state._board
    board_f = board.astype(jnp.float32)

    # Scaled board (28 elements)
    scaled_board = board_f / 15.0

    # Scaled dice (6 elements)
...
# =============================================================================
# Core game logic (unchanged from original)
# =============================================================================
...
def _is_action_legal(board: Array, action: Array) -> bool:
    ...
    return _is_to_point & _is_to_point_legal(board, src, tgt) | (~_is_to_point) & _is_to_off_legal(board, src, tgt, die)
...
def _is_to_off_legal(board: Array, src: int, tgt: int, die: int):
    ...
    return (src >= 0) & _exists(board, src) & _is_all_on_home_board(board) & ((d == die) | ((r <= die) & (r == d)))


def _is_to_point_legal(board: Array, src: int, tgt: int) -> bool:
    e = _exists(board, src)
    o = _is_open(board, tgt)
    return ((src >= 24) & e & o) | ((src < 24) & e & o & (board[_bar_idx()] == 0))
...
def _legal_action_mask_for_single_die(board: Array, die) -> Array:
    return (die == -1) * jnp.zeros(26 * 6, dtype=jnp.bool_) + (die != -1) * _legal_action_mask_for_valid_single_dice(board, die)
